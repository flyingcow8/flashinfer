...
def gen_fused_moe_sm80_module() -> JitSpec:
    ...
    SM80 supports:
    - FP16, BF16, FP32 activations and weights
    - UINT8, UINT4 quantized weights
    - Fused gated activation (Swiglu/Geglu)
    
    SM80 does NOT support:
    - FP8 (requires SM89+)
    - FP4 (requires SM90+)
    ...
    return gen_jit_spec(
        "fused_moe_sm80",
        [
            # Core MoE GEMM kernels for SM80
            jit_env.FLASHINFER_CSRC_DIR
            / "nv_internal/tensorrt_llm/kernels/internal_cutlass_kernels/src/moe_gemm/moe_gemm_kernels_fp16_fp16.cu",
            jit_env.FLASHINFER_CSRC_DIR
            / "nv_internal/tensorrt_llm/kernels/internal_cutlass_kernels/src/moe_gemm/moe_gemm_kernels_fp16_uint8.cu",
...
def get_fused_moe_sm80_module():
    ...
    class MoERunner(TunableRunner):
        ...
        def __init__(
            ...
            if instance_key not in MoERunner._runner_dict:
                MoERunner._runner_dict[instance_key] = (
                    torch.classes.fused_moe_sm80.FusedMoeRunner(
                        x_dtype,
                        weight_dtype,
                        output_dtype,
                        use_fp8_block_scaling,
    ...
    @register_fake_op("flashinfer::cutlass_fused_moe_sm80")
    def _fake_cutlass_fused_moe_sm80(
        output: torch.Tensor,
        input: torch.Tensor,
        token_selected_experts: torch.Tensor,
        token_final_scales: torch.Tensor,
    ...
    # Register the module
    return SimpleNamespace(
        cutlass_fused_moe_sm80=cutlass_fused_moe_sm80,
    )
...
def cutlass_fused_moe(
    ...
    Note:
        ...
        - Architecture-specific limitations:
          * SM80 (Ampere): Supports FP16, BF16, FP32, UINT8, UINT4. No FP8, FP4, or min_latency_mode.
          * SM90+ (Hopper/Blackwell): Supports all features including FP8, FP4, and min_latency_mode.
    """
    # Detect compute capability and choose appropriate backend
    ...
    if major == 8:  # SM80, SM86, SM87, SM89
        ...
        # Check for unsupported data types on SM80
        if input.dtype == torch.float8_e4m3fn or input.dtype == torch.float8_e5m2:
            raise NotImplementedError(
                f"FP8 input dtype {input.dtype} is not supported on SM80 (Ampere). "
                "FP8 requires SM89+ (Ada) for limited support or SM90+ (Hopper) for full support."
            )
        if fc1_expert_weights.dtype == torch.int64:  # NVFP4 marker
            raise NotImplementedError(
        ...
        # SM90+ backend (existing implementation)
        if use_fp8_block_scaling:
            raise NotImplementedError(
        ...
        raise ValueError(
            f"Unsupported compute capability SM{major}{minor}. "
            "FlashInfer fused MoE requires SM80+ (Ampere or newer)."
        )
